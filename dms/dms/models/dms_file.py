...
class DMSFile(models.Model):
    ...
    content_local = fields.Char(
        string="Local File Path",
        help="Relative path to the file on the local filesystem. "
        "Only used when the storage save type is 'Local Filesystem'.",
        prefetch=False,
    )

    content_sharepoint_path = fields.Char(
        string="SharePoint File Path",
        help="Server-relative URL of the file on SharePoint. "
        "Only used when the storage save type is 'SharePoint'.",
        prefetch=False,
    )
    ...
    def _get_content_inital_vals(self):
        return {
            ...
            "content_file": False,
            "content_local": False,
            "content_sharepoint_path": False,
        }

    def _get_local_filepath(self, relative_path=False):
        storage_path = self.storage_id.local_storage_path
        if not storage_path:
            raise UserError(_("Local storage path is not configured on the storage."))
        if relative_path:
    ...
    def _update_content_vals(self, vals, binary):
        ...
        elif self.storage_id.save_type == "local":
            new_vals["content_local"] = self._save_to_local(binary)
        elif self.storage_id.save_type == "sharepoint":
            new_vals["content_sharepoint_path"] = self._save_to_sharepoint(binary)
        else:
            new_vals["content_binary"] = self.content and binary
        return new_vals

    def _save_to_local(self, binary):
        storage_path = self._get_local_filepath()
        rel_dir = self._get_local_rel_dir()
        abs_dir = os.path.join(storage_path, rel_dir)
        os.makedirs(abs_dir, exist_ok=True)
        filename = "%s_%s" % (self.id or self._get_next_id(), self.name or "unnamed")
        safe_name = "".join(c for c in filename if c.isalnum() or c in "._- ")
        rel_path = os.path.join(rel_dir, safe_name)
        abs_path = os.path.join(storage_path, rel_path)
        with open(abs_path, "wb") as f:
            f.write(binary or b"")
        return rel_path

    def _get_local_rel_dir(self):
        return str(self.storage_id.id)

    @api.model
    def _get_next_id(self):
        return (self.env["dms.file"].search([], order="id desc", limit=1).id or 0) + 1

    def _save_to_sharepoint(self, binary):
        client = self.storage_id._get_sharepoint_client()
        if not client or not SPFile:
            raise UserError(
                _("SharePoint is not configured or office365 library is missing.")
            )
        library_name = self.storage_id.sharepoint_library_name
        if not library_name:
            raise UserError(_("SharePoint library name is not configured on the storage."))
        target_folder = client.web.lists.get_by_title(library_name).root_folder
        client.load(target_folder)
        client.execute_query()
        file_name = "%s_%s" % (
            self.id or self._get_next_id(),
            self.name or "unnamed",
        )
        safe_name = "".join(c for c in file_name if c.isalnum() or c in "._- ")
        target_folder.upload_file(safe_name, binary or b"")
        client.execute_query()
        return "/".join([target_folder.serverRelativeUrl, safe_name])

    def _delete_from_sharepoint(self):
        if not self.content_sharepoint_path:
            return
        client = self.storage_id._get_sharepoint_client()
        if not client:
            return
        file_obj = client.web.get_file_by_server_relative_url(self.content_sharepoint_path)
        file_obj.delete_object()
        client.execute_query()

    def _read_from_local(self, bin_size=False):
        if not self.content_local:
            return False
        abs_path = self._get_local_filepath(self.content_local)
        if not os.path.isfile(abs_path):
            return False
        with open(abs_path, "rb") as f:
            binary = f.read()
        if bin_size:
            return human_size(len(binary))
        return base64.b64encode(binary)

    def _read_from_sharepoint(self, bin_size=False):
        if not self.content_sharepoint_path or not SPFile:
            return False
        client = self.storage_id._get_sharepoint_client()
        if not client:
            return False
        response = SPFile.open_binary(client, self.content_sharepoint_path)
        if bin_size:
            return human_size(len(response.content))
    ...
    @api.depends("content_binary", "content_file", "attachment_id", "content_local", "content_sharepoint_path")
    def _compute_content(self):
        ...
        for record in self:
            ...
            elif record.content_local:
                record.content = record._read_from_local(bin_size)
            elif record.content_sharepoint_path:
                record.content = record._read_from_sharepoint(bin_size)

    @api.depends("content_binary", "content_file", "content_local", "content_sharepoint_path")
    def _compute_save_type(self):
        for record in self:
            if record.content_file:
                record.save_type = "file"
            elif record.content_local:
                record.save_type = "local"
            elif record.content_sharepoint_path:
                record.save_type = "sharepoint"
            else:
    ...
    def unlink(self):
        ...
        for record in self:
            if record.content_local:
                record._delete_local_file()
            if record.content_sharepoint_path:
                record._delete_from_sharepoint()
        res = super().unlink()
